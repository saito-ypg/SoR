...
void Sprite::Draw(Transform& transform, RECT rect, float alpha)
{
	...
	// テクスチャ座標変換行列を渡す
	XMMATRIX mTexTrans = XMMatrixTranslation((float)rect.left / (float)texSize.x,(float)rect.top / (float)texSize.y, 0.0f);
	XMMATRIX mTexScale = XMMatrixScaling((float)rect.right / (float)texSize.x,(float)rect.bottom / (float)texSize.y, 1.0f);
